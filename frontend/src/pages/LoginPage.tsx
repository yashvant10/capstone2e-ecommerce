...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post('/auth/supabase-login', { accessToken: data.session.access_token });
      ...
      console.error(err);
      toast.error(err.response?.data?.error?.message || err.message || 'Login failed. Please check your credentials.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-white">
      ...
      <div className="hidden lg:flex w-1/2 bg-brand-gradient flex-col justify-between p-12 relative overflow-hidden">
        {/* Decorative elements */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl translate-x-1/2 -translate-y-1/2 animate-float"></div>
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-purple-500/20 rounded-full blur-3xl -translate-x-1/2 translate-y-1/2 animate-float stagger-2"></div>
        ...
        <div className="relative z-10 max-w-md">
          <h1 className="font-display font-bold text-5xl text-white leading-tight mb-6 animate-fade-in-up">
            Welcome back to premium shopping.
          </h1>
          <p className="text-brand-100 text-lg animate-fade-in-up stagger-1">
            Sign in to access your exclusive wishlist, track your orders in real-time, and discover new arrivals curated just for you.
          </p>
        </div>
        
        <div className="relative z-10 text-brand-200 text-sm animate-fade-in-up stagger-2">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 relative">
        ...
        <div className="w-full max-w-md animate-fade-in">
          <div className="mb-10 text-center lg:text-left mt-12 lg:mt-0">
            <h2 className="font-display font-bold text-3xl md:text-4xl text-slate-900 mb-3">Sign in</h2>
          ...
          <div className="mt-10 text-center">
            <p className="text-slate-600">
              ...
              <Link to="/register" className="font-semibold text-brand-600 hover:text-brand-700 hover:underline underline-offset-4 transition-all">
                Create an account
              </Link>
            </p>
          </div>
